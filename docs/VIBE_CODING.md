...
The `config.aiProvider` field controls which path is taken. If `EXPO_PUBLIC_AI_PROVIDER=vercel` is explicitly set, it always wins even if a Gemini key is present.
