...
export const PhantomBackground = memo(function PhantomBackground({
  ...
  if (!isPhantom) {
    return (
      <View
        style={[{ flex: 1, backgroundColor: t.colors.neutral.darkest }, style]}
      >
        {children}
      </View>
    );
  ...
  const webStyle: WebViewStyle | null =
    Platform.OS === "web"
      ? {
  ...
  return (
    <View testID={testID} style={[styles.root, webStyle, style]}>
      <View style={styles.halftoneOverlay} pointerEvents="none" />
      {variant === "intense" && (
        <View style={styles.intenseOverlay} pointerEvents="none" />
      )}
      {children}
    </View>
  );
